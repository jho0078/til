...
public class Code07 {
	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			if (command.equals("create")) {
				...
				int index = find(name);
				if (index == -1) {
					System.out.println("No such poynomial exists.");
				}
				else {
	...
	private static int calcPolynomial(Polynomial2 p, int x) {
		int result = 0;
	...
	private static void printPolynomial(Polynomial2 p) {
		for (int i=0; i < p.nTerms; i++) {
			printTerm(p.terms[i]);
			System.out.print("+");
		}
	...
	private static void printTerm(Term2 term) {
		System.out.print(term.coef + "x^" + term.expo);
	}
	
	private static void addTerm(Polynomial2 p, int c, int e) {
	...
	private static int findTerm(Polynomial2 p, int e) {
